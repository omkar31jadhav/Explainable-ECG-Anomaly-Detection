...
def plot_ecg_with_explanation(series, explanation, title="ECG with Local Explanations"):
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(size=8, color=explanation, colorscale='Reds', showscale=True, colorbar=dict(title="Importance", thickness=15, len=0.8))
    ))
    fig.update_layout(
        title=title, 
        xaxis_title="Time", 
...
with st.expander("📊 Dataset Overview & Model Performance (Click to expand)", expanded=False):
    ...
    with sum_col2:
        ...
        fig_pie = px.pie(df_classes, values='Count', names='Class', title="Test Layout Distribution", height=250)
    ...
    with sum_col3:
        ...
        fig_loss.update_layout(title="Validation Loss Curve", height=250, margin=dict(l=20, r=20, t=40, b=20))
...
if 'series' in st.session_state:
    ...
    with pred_col2:
        ...
        fig_expl = plot_ecg_with_explanation(series, explanation, title="Input Signal + Explainability Overlay")
